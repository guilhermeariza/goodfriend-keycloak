...
<!-- GoodFriend Integration -->
<script type="module">
    import { GoodFriend } from '${resourceUrl}/js/goodfriend.js';
    
    console.log('GoodFriend module imported');
    
    try {
        GoodFriend.init({
             serverUrl: 'http://localhost:3001',
             context: 'You are an intelligent assistant integrated into the Keycloak 1.5.0.Final Admin Console. Your goal is to help the user configure this specific version of Keycloak. You are aware that this is an older version and should not suggest features from newer versions (like Authorization Services or distinct CIBA flows if not present). The user is learning Keycloak. Guide them through Realm creation, Client configuration, and User management.',
             uiOptions: { 
        ...
        console.log('GoodFriend initialized successfully');
    } catch (e) {
        console.error('Failed to initialize GoodFriend:', e);
    }
</script>
